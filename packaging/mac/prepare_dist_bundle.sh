...
do
	...
	$PYTHON_CURRENT/bin/python3.10  -s -m pip install -r $req_file
done
$PYTHON_CURRENT/bin/python3.10 -s -m pip install --no-cache-dir certifi
$PYTHON_CURRENT/bin/python3.10 -s -m pip install --no-cache-dir ../../
...
rm -rf $PYTHON_CURRENT/lib/python3.10/ensurepip
...
find $PYTHON_CURRENT/bin -type f -perm +111 -exec strip -S {} + 2>/dev/null || true
find $PYTHON_CURRENT/lib -name "*.so" -exec strip -S {} + 2>/dev/null || true
find $PYTHON_CURRENT/lib -name "*.dylib" -exec strip -S {} + 2>/dev/null || true
strip -S $PYTHON_CURRENT/Python 2>/dev/null || true

# for some reason the notarizer doesn't like txt files in these packages
if [ -d "$PYTHON_CURRENT/lib/python3.10/site-packages/lxml" ]; then
    find $PYTHON_CURRENT/lib/python3.10/site-packages/lxml -name '*.txt' -delete || true
fi
if [ -d "$PYTHON_CURRENT/lib/python3.10/site-packages/matplotlib" ]; then
