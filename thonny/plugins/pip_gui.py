        ...
        if self._is_read_only():
            ...
            if active_dist is not None:
                ...
                if self._is_read_only_package(name):
                    # It's installed but in a read-only location.
                    # We can still "upgrade" it by installing a newer version into our target directory.
    ...
    def _start_update_list(self, name_to_show=None):
        ...
        for dist in importlib.metadata.distributions():
            try:
                ...
                self._active_distributions[key] = DistInfo(
                    project_name=name,
                    key=key,
                ...
                logger.exception("Failed getting info for %s", dist)
    ...
    def _conflicts_with_thonny_version(self, req_strings):
        ...
        try:
            ...
            for req_string in req_strings:
                ...
                if req.name == "thonny" and thonny.get_version() not in req.specifier:
...
def _get_latest_stable_version(version_strings):
    def version_key(v):
        return [int(part) if part.isdigit() else part for part in re.split(r"([0-9]+)", v)]

    stable_versions = []
    for s in version_strings:
        if s.replace(".", "").isnumeric():  # Assuming stable versions have only dots and numbers
            stable_versions.append(s)

    if len(stable_versions) == 0:
        return None

    return sorted(stable_versions, key=version_key)[-1]
