...
def _handle_toplevel_response(event):
    if "AssistantView" not in get_workbench()._view_records:
        # Assistant plugin not yet initialized or view not registered
        return

    try:
        view = get_workbench().get_view("AssistantView", create=False)
    except RuntimeError:
        view = None

    if view is not None:
        view.handle_toplevel_response(event)
    else:
        if (
            event.get("user_exception")
            and get_workbench().get_option("assistance.open_assistant_on_errors")
        ) or get_workbench().get_option("assistance.open_assistant_on_warnings"):
            view = get_workbench().get_view("AssistantView")
            view.handle_toplevel_response(event)
